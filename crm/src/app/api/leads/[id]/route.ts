...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { stage: newStage, vehicleSpecialty, tags, ...rest } = body
  ...
  const lead = await db.lead.update({
    ...
    data: {
      ...
      ...(vehicleSpecialty ? { vehicleSpecialty: JSON.stringify(vehicleSpecialty) } : {}),
  ...
  if (newStage && newStage !== current.stage) {
    await db.activity.create({
      data: { leadId: params.id, type: 'STAGE_CHANGED', note: `Stage changed from ${current.stage} to ${newStage}` },
    })
  }
